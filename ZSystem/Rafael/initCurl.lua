...
local curl = require("ZSystem\\Rafael\\lib\\curl")
...
R_Request = {
	...
	send = function(url, args)
		...
		if (args.files) then
			...
			for key, value in pairs(args.files) do
				...
				local res =
					...
					post,
					lastptr,
					ffi.new("int", curl.CURLFORM_COPYNAME),
					key,
					ffi.new("int", curl.CURLFORM_FILE),
					file,
		...
		if (code ~= curl.CURLE_OK) then
			...
			return false, R_Request.error.unknown, "Unknown error", num
		end
		...
		if (out_buffer or headers_buffer) then
			...
			if cookielist[0] ~= nil then
				...
				repeat
					...
					local domain, subdomains, path, secure, expiration, name, value =
						raw:match("^(.-)\t(.-)\t(.-)\t(.-)\t(.-)\t(.-)\t(.*)$")
